...
If you're switching from iDEAL v2 to v3, see upgrading below
...
<pre><code>/usr/bin/openssl genrsa -aes128 -out private_key.pem -passout pass:the_passphrase 2048
/usr/bin/openssl req -x509 -sha256 -new -key private_key.pem -passin pass:the_passphrase -days 1825 -out private_certificate.cer</code></pre>

h3. Other

<pre><code>/usr/bin/openssl genrsa -des3 -out private_key.pem -passout pass:the_passphrase 1024
/usr/bin/openssl req -x509 -new -key private_key.pem -passin pass:the_passphrase -days 3650 -out private_certificate.cer</code></pre>
...
* "Test dashboard":https://idealtest.rabobank.nl
* "Live dashboard":https://ideal.rabobank.nl
...
class PaymentsController < ActionController::Base
  def create
    payment = Payment.create(params[:payment].merge(
      :price_in_euro => "10.00",                   # €10.00 formatted as: sprintf("%.2f", price)
...
h3. ING iDEAL v2 -> v3
  ...
  * Get the new certificate from the iDEAL dashboard
  * Generate your own new certificates
  * change the issuer_id field to text/varchar, this field contains the BIC codes now
  * Make sure the amount is in euro like 10.00 and not in cents!
